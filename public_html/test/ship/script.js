...
function moveStuff() {
	...
	for (x in fireballs) {
		...
		fb.yy += fb.motionY*3;
		var mot = fb.age / 10;
		fb.motionX = (fb.xx < posX) ? (fb.motionX + mot) : (fb.motionX - mot);
	...
	for (s in ships) {
		...
		sp.xx += (3*sp.motionL)/sp.heavy;
		sp.xx -= (3*sp.motionR)/sp.heavy;
...
function ship(type, whereX, whereY) {
	...
	//Evolving stats
	this.customValueX = 255*Math.random();
	this.customValueV = 255*Math.random();
	this.customValueW = 255*Math.random();
...
function fireball(type, from, strength, fworkMode) {
	...
	this.xx = posX+16;
	this.yy = 460;
	this.motionX = Math.random()*rand(-0.25,0.25);
	this.motionY = -6
